            ...
            <ListView class="list-group" for="item in allCards"
                ...
                <v-template>
                    <GridLayout @tap="showDetailPageModally(item)"
                        :visibility="item.visibility == 'visible' ? 'collapsed' : 'visible'"
        ...
        template: `
            <ModalStack class="modal-container">
                <GridLayout class="card modal" rows="auto,auto" verticalAlignment="middle">
                    <Button row="0" @tap="$modal.close" class="fa close" :text="'fa-times' | fonticon" horizontalAlignment="right" />
    ...
    export default {
        ...
        methods: {
            ...
            toggleItem(item) {
                ...
                } else if (item.visibility == "visible") {
